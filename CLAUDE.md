...
### V20 Bug 修复（2026-05-06）

**FIX-01 —— 编辑任务无法删除全部负责人**

- **位置**：`js/pm-tasks.js` — `submitEditTask()` 第 635 行
- **问题**：移除所有负责人后保存，`assignees` 回退到旧值：`t.assignees = newAssignees.length > 0 ? newAssignees : (t.assignees || []);`
- **修复**：改为 `t.assignees = newAssignees;`，允许清空负责人列表

**FIX-02 —— 负责人 chip 删除按钮图标不刷新**

- **位置**：`js/pm-tasks.js` — `addTaskAssignee()`、`removeTaskAssignee()`、`renderEditTaskAssigneeList()`
- **问题**：通过 JS 更新 chip 列表 innerHTML 后，`<i data-lucide="x">` 元素未被 lucide 重新扫描，导致删除按钮图标无法渲染为 SVG
- **修复**：三处 innerHTML 赋值后均添加 `if (typeof lucide !== "undefined") lucide.createIcons();`

---

## V21 迭代更新（2026-05-07）

根据 `PM_Board_V21_PRD.docx` 完成全部 15 项变更，涵盖 5 个类别。

### FIX-DOUBLE-RENDER · 修复 7 个函数的双重渲染

**问题**：Supabase 实时回调与显式 `render()` 调用竞态导致连续两次刷新（闪烁）。

**修复**：在以下 7 个函数中，`render()` / `switchView()` 前设置 `_lastLoadTime = Date.now()`，利用 `loadState()` 中 800ms 防抖阻止实时回调的二次渲染。

| 函数 | 文件 | 位置 |
|------|------|------|
| `submitEditTask()` | pm-tasks.js | `await syncTask(t)` 后，`render()` 前 |
| `submitEditProject()` | pm-projects.js | `await syncProject(p)` 后，`render()` 前 |
| `confirmDeleteProject()` | pm-projects.js | 过滤项目后，`switchView('today')` 前 |
| `removeMemberFromProject()` | pm-projects.js | `await syncProject(p)` 后 |
| `submitAddMember()` | pm-members.js | `state.members.push(newMember)` 后 |
| `deleteMember()` | pm-members.js | 过滤成员后，DOM 刷新前 |
| `deleteTag()` | pm-members.js | `buildTagsListHTML()` 调用后 |
...
| 变更 | 函数 | 说明 |
|------|------|------|
| 连续添加 | `submitAddTag()` | 保存后不清空输入框，保持焦点，可连续添加标签 |
| 删除保持 | `deleteTag()` | 删除后不关闭弹窗，继续管理标签（此前已正确实现） |
| 图标刷新 | `submitAddTag()`、`confirmEditTag()`、`deleteTag()` | innerHTML 更新后调用 `lucide.createIcons()` 刷新图标 |

### UX 修复（UX-01 ~ UX-10）

| 编号 | 说明 | 文件 | 变更 |
|------|------|------|------|
| UX-01 | 新建任务负责人不延续 | pm-tasks.js | `openAddTask()` 已有 `window._newTaskAssignees = []`（无需修改） |
| UX-02 | 编辑任务负责人不延续 | pm-tasks.js | `openEditTask()` 开头添加 `window._editTaskAssignees = []` |
| UX-03 | 筛选后列表滚回顶部 | pm-views.js | `renderTaskList()` 中 `innerHTML` 赋值后添加 `document.getElementById('main-content').scrollTop = 0` |
| UX-04 | 删除成员后图标不刷新 | pm-members.js | `deleteMember()` DOM 刷新后添加 `lucide.createIcons()` |
| UX-05 | 移除项目成员弹窗闪烁 | pm-projects.js | `removeMemberFromProject()` 改为内联 DOM 刷新，不再重开弹窗 |
| UX-06 | 甘特图缩放保持折叠 | pm-views.js | `setGanttZoom()` 已保留 `_ganttCollapsed`（无需修改） |
| UX-07 | Toast 在 modal 下方 | pm.css | `.toast` z-index 从 `9999` 改为 `99999` |
| UX-08 | 周视图「+N 更多」可点击 | pm-views.js, pm.css | `.week-more-label` 添加 onclick → `window.showWeekDayTasks(dayStr)` 弹出当日完整任务列表；CSS 添加 `cursor:pointer` + hover 效果 |
| UX-09 | 甘特图清除筛选不全 | pm-views.js | 清除按钮同时重置全部 4 个筛选变量（status、assignee、project、module） |
| UX-10 | 金额输入弹出数字键盘 | finance-t2.js, finance-t3.js | 快捷录入弹窗的金额输入框添加 `inputmode="decimal"` |

### FIX-GANTT-SCROLL · 甘特图滚动高度

**位置**：`css/pm.css`

`.gantt-wrap` 添加 `max-height: calc(100vh - 220px)`，防止甘特图区域超出屏幕导致整体页面滚动。

### FEAT-GANTT-FILTER · 甘特图项目+模块联动筛选

**位置**：`js/pm-views.js` — `renderGantt()`

**新增全局变量**：
```js
window._ganttFilterProject   // 项目筛选（falsy = 全部）
window._ganttFilterModule    // 模块筛选（falsy = 全部）
```

**工具栏新增**：在状态/负责人筛选前增加两个联动下拉框：
- **项目筛选** `<select>` — 选项来自 `state.projects`，切换时重置模块筛选
- **模块筛选** `<select>` — 选项来自 `state.modules`，联动当前选中项目（仅显示该项目下的模块）

**筛选逻辑**（两处同步）：
1. `groups.forEach` 渲染循环：`if (window._ganttFilterProject && g.proj.id !== ...) return;` + `if (window._ganttFilterModule) tasks = tasks.filter(t => t.moduleId === ...);`
2. `_ganttVisibleCount` 计数循环：同上

**清除按钮**：重置全部 4 个筛选变量（`_ganttFilterStatus`、`_ganttFilterAssignee`、`_ganttFilterProject`、`_ganttFilterModule`）

**`_hasFilter`** 计算包含全部 4 个变量。

### V21 新增函数

| 函数 | 位置 | 说明 |
|------|------|------|
| `window.showWeekDayTasks(dayStr)` | pm-views.js | 周视图「+N 个」点击后弹出当日完整任务列表弹窗 |
